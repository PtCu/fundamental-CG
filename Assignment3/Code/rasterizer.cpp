...
void rst::rasterizer::draw(std::vector<Triangle *> &TriangleList)
{
    ...
    for (const auto &t : TriangleList)
    {
        ...
        std::transform(mm.begin(), mm.end(), viewspace_pos.begin(), [](auto &v) { //std::transform在指定的范围内应用于给定的操作，并将结果存储在指定的另一个范围内(viewspace_pos)
...
//传递view_pos视角信息，方便后续着色
//view_pos是三角形顶点经过mv变化(没映射到[-1,1]以及屏幕)时的坐标，即为人眼在原点，三角形顶点在无限制坐标系中时的观测点。
//有了观测点信息可以方便的利用bling-phong模型等进行shading
        ...
        for (int x = min_x; x <= max_x; x++)
        {
            for (int y = min_y; y <= max_y; y++)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    {
                        tup = computeBarycentric2D(x + pos[i][0], y + pos[i][1], t.v);
                ...
                if (count)
                {
                    if (min_depth < depth_buf[get_index(x, y)])
                    {
                        ...
                        for (int i = 0; i < 3; i++)
                        {
                            interpolated_color[i] *= (count / 4.0);
                        }
